...
def root() -> JSONResponse:
    return JSONResponse(
        {
            "ok": True,
            "message": "POST to /remove-white as multipart/form-data with either 'file' or 'url'",
            "example": {
                "endpoint": "/remove-white",
                "method": "POST",
                "form_fields": {
                    "file": "<binary image> (mutually exclusive with 'url')",
                    "url": "<image url> (mutually exclusive with 'file')",
                    "threshold": 245,
                    "color_tolerance": 18,
                    "edge_only": True,
                    "feather": 0.8,
